...
def _step5_preserve_L_write_M(df: pd.DataFrame, step4_col: str) -> pd.DataFrame:
    """
    Προσπαθεί να χρησιμοποιήσει step5_enhanced.step5_place_remaining_students (διορθωμένη).
    Αν ΔΕΝ βρεθεί, τότε βάζει M = L (χωρίς αλλαγές, απλά διατηρείται η L).
    """
    out = df.copy()
    # Προσπάθεια import
    try:
        p = Path("step5_enhanced.py")
        if p.exists():
            m5 = _imp("step5_enhanced_single", p)
            updated, _score = m5.step5_place_remaining_students(out.copy(), scenario_col=step4_col, num_classes=None)
            return updated
    except Exception as e:
        print("Warning: Step5 module not found or failed, will set M=L. Error:", e)
    # Fallback: M = L
    N = _idx(step4_col)
    step5_col = f"ΒΗΜΑ5_ΣΕΝΑΡΙΟ_{N}"
    out[step5_col] = out[step4_col]
    return out

# ========= Main exporter (NO AUDIT) =========
def export_single_noaudit(in14_path: str, out_path: str):
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as w:
        for s in sheets:
            ...
            df5 = _step5_preserve_L_write_M(df.copy(), s4)
